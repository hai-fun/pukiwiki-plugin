...
class PluginReplacepluginView
{
	...
	function preview($apage, $diff)
	{
		...
		$msg .= '<pre>' . diff_style_to_css(htmlsc($diff)) . '</pre>'; // Pukiwiki API
		...
		if (empty($text)) {
			...
			if (UI_LANG == "ja") {
				$text['label'] = array(
					 ...
					 'except'	  => _('除外ページ'),
					 'page'		=> _('ページ'),
					 'oldplugin'   => _('古いプラグイン名'),
					 'newplugin'   => _('新しいプラグイン名'),
					 'plugintype'  => _('プラグインタイプ'),
					 'oldoption'   => _('古いオプション名'),
					 'optionnum'   => _('オプション引数'),
					 'newoption'   => _('新しいオプション名'),
					 'regexp'	  => _('正規表現'),
					 'notimestamp' => _('日時更新しない'),
					 'preview'	 => _('プレビュー'),
				);
			}
		}
