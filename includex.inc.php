...
class PluginIncludex
{
	...
	function get_titlestr($inclpage, $option = null, $current = null)
	{
		...
		case 'off':
			...
			break;
		case 'name':
			$titlestr = htmlsc($inclpage);
			break;
		case 'basename':
			$titlestr = htmlsc(basename($inclpage));
			break;
		case 'relname':
			if (! isset($current)) $current = $GLOBALS['vars']['page'];
			if (($i = strpos($inclpage, $current . '/')) === 0) {
				$titlestr = htmlsc(substr($inclpage, strlen($current)+1));
			} else {
				$titlestr = htmlsc($inclpage);
			}
			break;
		...
		default:
			...
			if ($titlestr == '') $titlestr = htmlsc($inclpage);
			break;
		}
		return $titlestr;
	}
	...
	// PukiWiki API InlineConverter does htmlsc, too. 
